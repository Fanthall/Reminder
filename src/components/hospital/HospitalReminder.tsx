...
const HospitalReminder = () => {
    const navigation = useNavigation();
    const darkMode = useColorScheme()

    return (
        <View style={[styles.safeAreaView, { backgroundColor: (darkMode === "dark" ? AppColors.grayDarker : AppColors.white) }]}>
            <Header left={<BackButton />} headerText={language.render("hospitalReminder")} />
            <ScrollView style={styles.scrollView}>
            <Text >{language.render("hospitalReminder")}</Text>                
            </ScrollView>
        </View>
    );
}
...
export default HospitalReminder;